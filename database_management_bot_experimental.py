...
@under_maintenance
def register_experimental(update: Update, context: CallbackContext) -> int:
    """
...
@under_maintenance
def get_admin_photo_experimental(update: Update, context: CallbackContext) -> int:
    """
    EXPERIMENTAL FUNCTION
    """
    ...
    context.user_data["phone_number_markdown"] = update.message.contact.phone_number.replace('+', '\+')
    ...
    update.message.reply_text(
        text='👤 *Тепер, надішліть, будь ласка, фото свого обличчя*'
             ...
             ' Диверсанти не мають мати доступу до такої інформації\.'
...
@under_maintenance
def add_admin_experimental(update: Update, context: CallbackContext) -> ConversationHandler.END:
    """
    EXPERIMENTAL FUNCTION
    """
    user_id = update.effective_user.id
    ...
    update.message.reply_photo(
        face,
        caption=f"✅ *{user.first_name}*, Вас успішно зареєстровано як адміністратора"
                f"\n\nВаш ID: *{user_id}*"
                f"\nВаш номер телефону: *{context.user_data['phone_number_markdown']}*",
        parse_mode="MarkdownV2",
        reply_markup=ReplyKeyboardMarkup(reply_keyboard,
                                         resize_keyboard=True,
                                         input_field_placeholder='Оберіть опцію')
...
def main() -> None:
    ...
    register_handler_experimental = ConversationHandler(
        entry_points=[CommandHandler('authorize', register_experimental)],
        states={
            CONTACT: [
                MessageHandler(Filters.contact & ~Filters.command, get_admin_photo_experimental)
            ],
            ADMIN_PHOTO: [
                MessageHandler(Filters.photo & ~Filters.command, add_admin_experimental)
            ],
        },
    ...
    dispatcher.add_handler(register_handler_experimental)
